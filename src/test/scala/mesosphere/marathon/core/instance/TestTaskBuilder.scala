...
case class TestTaskBuilder(
  ...
  def mesosStatusForCondition(condition: Condition, taskId: Task.Id): Option[mesos.Protos.TaskStatus] = condition match {
    ...
    case Condition.UnreachableInactive => Some(MesosTaskStatusTestHelper.unreachable(taskId)) // TODO handle inactive correctly
  }
  ...
  def withNetworkInfo(networkInfo: NetworkInfo): TestTaskBuilder = {
    val foo = copy(task = task.map(_.withNetworkInfo(networkInfo)))
    foo
  }
  ...
  def withNetworkInfo(
    ...
    this.copy(task = task.map(_.withNetworkInfo(hostName, hostPorts, networkInfos)))
