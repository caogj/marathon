...
class RetryTest extends AkkaUnitTest {
  ...
  "Retry" when {
    "async" should {
      ...
      "retry should timeout after max total time" in {
        val random: Option[Random] = Some(new Random(0))
        val counter = new AtomicInteger()
        val ex = new Exception
        val result = util.Retry(
          "failure",
          maxAttempts = Int.MaxValue,
          maxTotalTime = 2.seconds,
          minDelay = 1.nano,
          maxDelay = 1.second,
          random = random) {
          countCalls(counter)(Future.failed(ex))
    ...
    "randomBetween" should {
      "always return a value between min and max" in {
        (1L to 100).foreach { i =>
          val max = i * 100
          val rand = Retry.randomBetween(i, max, new Random(1L))
          rand should be <= max
        }
      }
      "return the same result for (x,y) and (y,x)" in {
        (1L to 100).foreach { i =>
          val max = i * 100
          val res1 = Retry.randomBetween(x = max, y = i, random = new Random(1L))
          val res2 = Retry.randomBetween(x = i, y = max, random = new Random(1L))
      ...
      "never return a delay bigger than maxDelay" in {
        ...
        (1 to 100).foreach { i =>
          val nextDelay = Retry.computeNextDelay(max, last = i.nanos, random = random)
